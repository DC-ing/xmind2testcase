...
from xmind2testcase.maker_ray_excel import xmind_to_excel_file
...
here = os.path.join(current_file_path, "data")
...
# 设置werkzeug_logger的propagate属性为False，防止传播到root_logger
...
def init():
    ...
    if not exists(DATABASE):
        init_db()
    app.logger.info(
        "Congratulations! the xmind2testcase webtool database has initialized successfully!"
    )
...
def delete_records(keep=20):
    ...
    sql = "SELECT * from records where is_deleted<>1 ORDER BY id desc LIMIT -1 offset {}".format(
        keep
    )
...
def get_records(limit=8):
    ...
    c = g.db.cursor()
    sql = "select * from records where is_deleted<>1 order by id desc limit {}".format(
        int(limit)
    )
    c.execute(sql)
    rows = c.fetchall()
    ...
    for row in rows:
        name, short_name, create_on, note, record_id = (
            row[1],
            row[1],
            row[2],
            row[3],
        ...
        create_on = time_to_chinese(create_on)
...
def save_file(file):
    if file and allowed_file(file.filename):
        ...
        if exists(upload_to):
            filename = "{}_{}.xmind".format(
                filename[:-6], arrow.now().strftime("%Y%m%d_%H%M%S")
            )
            upload_to = join(app.config["UPLOAD_FOLDER"], filename)
...
def download_testlink_file(filename):
    ...
    filename = (
        os.path.basename(testlink_xmls_file) if testlink_xmls_file else abort(404)
    )
