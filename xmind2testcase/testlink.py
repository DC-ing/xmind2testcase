...
from xmind2testcase.parser import config_sep
...
def gen_testcase_element(suite_element, suite):
    for testcase in suite.testcase_list:
        ...
        gen_text_element(
            testcase_elment, const.TAG_PRECONDITIONS, testcase.preconditions
        )
        gen_text_element(
            testcase_elment,
...
def is_should_parse(content):
    ...
    return (
        ...
        and content.strip() != ""
        and not content[0] in config_sep["ignore_char"]
    )
...
def is_should_skip(content):
    ...
    return (
        ...
        or content.strip() == ""
        or content[0] in config_sep["ignore_char"]
    )
