...
from xmind2testcase.config import Priority, TestResult
...
def xmind_to_testsuites(xmind_content_dict):
    ...
    for sheet in xmind_content_dict:
        ...
        if sub_topics:
            ...
            logging.warning(
                "This is a blank sheet(%s), should have at least 1 sub topic(test suite)",
                sheet["title"],
            )
            continue
...
def filter_empty_or_ignore_topic(topics):
    ...
    result = [
        ...
        if not (
            ...
            or topic["title"][0] in config_sep["ignore_char"]
        )
    ]
...
def filter_empty_or_ignore_element(values):
    ...
    for value in values:
        if (
            ...
            and not value.strip() == ""
            and not value[0] in config_sep["ignore_char"]
        ):
...
def sheet_to_suite(root_topic):
    ...
    if separator in config_sep["valid_sep"]:
        logging.debug(
            "find a valid separator for connecting testcase title: %s", separator
        )
        config_sep["sep"] = separator  # set the separator for the testcase's title
...
def parse_a_testcase(case_dict, parent):
    ...
    testcase.sub_suite = gen_testsuite_title(parent)
    ...
    # TODO 用例编写人字段与原有执行类型字段冲突
    testcase.writer = case_dict.get("label") or testcase.writer
    testcase.execution_type = get_execution_type(topics)
    testcase.importance = get_priority(case_dict) or Priority.B.name
    ...
    if testcase.result == TestResult.NON_EXECUTION.val and testcase.steps:
        for step in testcase.steps:
            if step.result == TestResult.FAILED.val:
                testcase.result = TestResult.FAILED.val
                break
            if step.result == TestResult.BLOCKED.val:
                testcase.result = TestResult.BLOCKED.val
                break

            # there is no need to judge where test step are ignored
    ...
    if isinstance(case_dict["markers"], list):
        for marker in case_dict["markers"]:
            if marker.startswith("priority"):
                return Priority.get_priority(int(marker[-1]))
                # return int(marker[-1])


def gen_testsuite_title(topics):
...
def gen_testcase_preconditions(topics):
    ...
    return config_sep["precondition_sep"].join(notes)
...
def gen_testcase_summary(topics):
    ...
    return config_sep["summary_sep"].join(comments)
...
def parse_a_test_step(step_dict):
    ...
    if expected_topics:  # have expected result
        ...
        test_step.expectedresults = expected_topic[
            "title"
